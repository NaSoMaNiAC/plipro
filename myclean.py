...
class Database:
    def __init__(self, db):
        ...
        self.cur.execute(
            "CREATE TABLE IF NOT EXISTS καθαρα (item_name text, item_price float, purchase_date date)")
        self.conn.commit()
    ...
    def insertRecord(self, item_name, item_price, purchase_date):
        self.cur.execute("INSERT INTO καθαρα VALUES (?, ?, ?)",
    ...
    def updateRecord(self, item_name, item_price, purchase_date, rid):
        self.cur.execute("UPDATE καθαρα SET item_name = ?, item_price = ?, purchase_date = ? WHERE rowid = ?",
